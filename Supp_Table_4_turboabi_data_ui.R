...
ui <- fluidPage(tabsetPanel(
    ...
    tabPanel("Data overview",
             ...
             #               mainPanel(
                               ...
                               "This is a user interface to explore and interpret mass spectrometry data generated from TurboID-ABI1 proximity labeling experiments reported in Petersen et al (2022)", br(), br(),
                            #   "1) Upload file from supplemental info: 'Supp_File_5_cleaned_data.csv'", br(),
                               "1) Adjust statistical filtering on tab: 'Volcano Plots'", br(),
                               "2) Initiate new analyses by clicking tabs and waiting for load time", br(),
    ...
    tabPanel("Data table",
             ...
             mainPanel(
                 ...
                 downloadButton("download_datatable", "Download filtered data to downloads folder (please don't change file name)")
             ))
))
...
server <- function(input, output,session) {
    ...
    data <- reactive({read.csv("Supp_File_5_cleaned_data.csv")})
    as.data.frame(data)
      ...
      #  req(input$dataset)
        ...
        p <- ggplot(data = filtered_data_reactive(), aes(x = log2(as.numeric(as.character(ratio_abi_turbo_abundance))),
                                                         ...
                                                         text=if (input$tails == "one"){paste("Protein: ", gene, "\n",
                                                                                              "TurboAbi vs. TurboControl PA (% / ratio): ", percentage_abi_turbo_abundance, "/", ratio_abi_turbo_abundance, "\n",
                                                                                              "TurboAbi vs. TurboControl PA (% / ratio): ", percentage_abi_wt_abundance, "/", ratio_abi_wt_abundance, "\n",
                                                                                              "TurboAbi vs. TurboControl PSM (% / ratio): ", percentage_abi_turbo_psm, "/", ratio_abi_turbo_psm, "\n",
                                                                                              "TurboAbi vs. WT PSM (% / ratio): ", percentage_abi_wt_psm, "/", ratio_abi_wt_psm, "\n",
                                                                                              "TurboAbi vs. TurboControl FDR: ", qval_abi_turbo_abundance, "\n",
                                                                                              "TurboAbi vs. TurboControl p-value", pval_abi_turbo_abundance, "\n",
                                                                                              "TurboAbi vs. WT FDR: ", qval_abi_turbo_abundance)}
                                                         else if (input$tails == "two"){paste("Protein: ", gene, "\n",
                                                                                              "TurboAbi vs. TurboControl PA (% / ratio): ", percentage_abi_turbo_abundance, "/", ratio_abi_turbo_abundance, "\n",
                                                                                              "TurboAbi vs. WT PA (% / ratio): ", percentage_abi_wt_abundance, "/", ratio_abi_wt_abundance, "\n",
                                                                                              "TurboAbi vs. TurboControl PSM (% / ratio): ", percentage_abi_turbo_psm, "/", ratio_abi_turbo_psm, "\n",
                                                                                              "TurboAbi vs. WT PSM (% / ratio): ", percentage_abi_wt_psm, "/", ratio_abi_wt_psm, "\n",
                                                                                              "TurboAbi vs. TurboControl FDR: ", two_tailed_qval_abi_turbo_abundance, "\n",
                                                                                              "TurboAbi vs. TurboControl p-value", two_tailed_pval_abi_turbo_abundance, "\n",
                                                                                              "TurboAbi vs. WT FDR: ", two_tailed_qval_abi_turbo_abundance)} 
        ),
        interactors = paste(length(gene), "interactors")) +
            ...
            labs(x = "log2(TurboAbi:TurboControl PA ratio)",
                 y = "-log10(TurboAbi:TurboControl PA p-value)",
     ...
     #   req(input$dataset)
        ...
        p <- ggplot(data = filtered_data_missing_control_reactive(), aes(x = as.numeric(as.character(rsd_abi_abundance)),
                                                                         ...
                                                                         text = if (input$tails == "one"){paste("Protein: ", gene, "\n",
                                                                                                                ...
                                                                                                                "TurboAbi vs. TurboControl FDR: ", qval_abi_turbo_abundance, "\n",
                                                                                                                "TurboAbi vs. TurboControl p-value", pval_abi_turbo_abundance, "\n",
                                                                                                                "TurboAbi vs. WT FDR: ", qval_abi_wt_abundance)}
                                                                         else if (input$tails=="two"){paste("Protein: ", gene, "\n",
                                                                                                            "TurboAbi vs. TurboControl PA (% / ratio): ", percentage_abi_turbo_abundance, "/", ratio_abi_turbo_abundance, "\n",
                                                                                                            "TurboAbi vs. WT PA (% / ratio): ", percentage_abi_wt_abundance, "/", ratio_abi_wt_abundance, "\n",
                                                                                                            "TurboAbi vs. TurboControl PSM (% / ratio): ", percentage_abi_turbo_psm, "/", ratio_abi_turbo_psm, "\n",
                                                                                                            ...
                                                                                                            "TurboAbi vs. TurboControl FDR: ", two_tailed_qval_abi_turbo_abundance, "\n",
                                                                                                            "TurboAbi vs. TurboControl p-value", two_tailed_pval_abi_turbo_abundance, "\n",
                                                                                                            "TurboAbi vs. WT FDR: ", two_tailed_qval_abi_wt_abundance)}),
                    interactors = paste(length(gene), "interactors")) +
            geom_point(aes(fill=known_interactor, size=2)) +
